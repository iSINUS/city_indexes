INSERT INTO ${SQL_INDEX_NAME}_index_full (
	WITH
		${SQL_INDEX_NAME}_index_10 AS (
			SELECT a.city, a.h3_10 AS h3, 10::SMALLINT AS zoom, SUM(POWER((${SQL_INDEX_RADIUS}-h3_grid_distance(a.h3_10,b.h3_10)),2)*b.area) AS ${SQL_INDEX_NAME}_index
			FROM city_h3 a
		...
		${SQL_INDEX_NAME}_index_9 AS (
			SELECT a.city, a.h3_9 AS h3, 9::SMALLINT AS zoom, SUM(POWER((${SQL_INDEX_RADIUS}-h3_grid_distance(a.h3_10,b.h3_10)),2)*b.area) AS ${SQL_INDEX_NAME}_index
			FROM city_h3 a
			JOIN ${SQL_INDEX_SOURCE} b ON a.city=b.city AND b.h3_10=a.h3_10_
			WHERE b.h3_10 IS NOT NULL AND b.city='${SQL_INDEX_CITY}'
			GROUP BY 1,2),
